...
export default class AdminProfileLWR extends LightningElement {
    /* Decalare Variables */
    ...
    @wire(getDriverDetails, {
        ...
        if (data) {
            ...
            console.log("getDriverDetails###", JSON.parse(data), data)
        } else if (error) {
    ...
    constructor() {
        ...
        this.popStateMessage = this.popStateMessage.bind(this);
    }
    ...
    handleLogout() {
        ...
        location.href = "/appV1/secur/logout.jsp";
    }
    ...
    popStateMessage = (event) => {
        ...
        if (address === "#Mileage-Approval") {
            ...
            this.getRolesAndManagers(this.accountId);
    ...
    showToast(event) {
        ...
        this.refs.toast.fireToastr(content.message, content.type);
        /*this.dispatchEvent(
    ...
    refreshSync() {
        ...
        this.unapproveMileages = "";
        console.log(this.accountId, this.contactId)
        let team = this.isTeamShow === "false" ? false : true;
        getAllDriversLastMonthUnapprovedReimbursementsclone({
            accountId: this.accountId,
            contactId: this.contactId,
    ...
    getAllUnapprove(boolean) {
        console.log(boolean, this.userRole, this.accountId, this.contactId)
        getAllDriversLastMonthUnapprovedReimbursementsclone({
            accountId: this.accountId,
            contactId: this.contactId,
            showTeam: boolean,
    ...
    getMileageHighRisk(boolean, v, monthName) {
        console.log("Mileage risk", boolean, v, monthName, this.userRole)
        const url = new URL(location);
        let address = url.hash;
        this.notificationViewClicked = false;
    ...
    getDriverList() {
        ...
        dropdownDriverName({ accountId: this.accountId, managerId: this.contactId, role: this.userRole })
            .then((data) => {
    ...
    getEmployeeList() {
        getlistAllEmployees({ accid: this.accountId, contactid: this.contactId })
            .then(response => {
    ...
    getMileageData(){
        fetchMileages({accID : this.accountId ,AdminId: this.contactId ,limitSize : 2000, offset : 0})
        .then((result) => {
           this.mileageRecord = JSON.stringify(result)
           console.log("Mileages", result);
        })  
        .catch((error) => {
          console.log("Mileages error", error.message)
        })
    }
    ...
    redirectToDriverView(event) {
        ...
        document.title = "Team";
        location.href = location.origin + location.pathname + location.search + '#Driver-view'
    }
    ...
    handleUserNavigate(event) {
        ...
        location.href = location.origin + location.pathname + location.search + '#Users'
    }
    ...
    getListOfDropDownData() {
        ...
        getPickListValuesIntoList({ accid: this.accountId })
            .then(responce => {
